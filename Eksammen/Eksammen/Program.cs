...
namespace ProeveEksammen
{
    class Program
    {
        ...
        {
            // Array af menu items
            var menuItems = new String[] { "AB (Almindelig billet)", "ABK (Almindelig billet med adgang til skatkammeret)", "BK (Billet til skatkammeret)" };
            // Array af priser i samme order som menu items
            var menuPrices = new double[] { 75, 125 ,25};
            ...
            do
            {
                try
                {
                    Console.WriteLine("Du har valgt {0} hvor mange bileter ønsker du: ",menuItems[currentMenuItem]);
            ...
            // Udregner totalprisen i usd for valgte menu item i euro
            var totalPrisUsd = totalPris / 7.42;
            // Fortæller brugen hvad prisen er
            Console.WriteLine("Du har bestilt {0} {1} til {2} kr eller {3:n2} usd", amount, menuItems[currentMenuItem], totalPris, totalPrisUsd);
        }
    }
}